...
window.productCard = (product) => `<article class="product-card"><a href="./product.html?id=${product.id}">${window.productVisual(product)}<div class="product-card-body"><span class="condition-tag condition-${product.condition.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}">${product.condition}</span><h3>${product.brand} ${product.model}</h3><p>${product.storage}${product.ram ? ` · ${product.ram}` : ''}</p><strong>${window.formatPrice(product.priceKes)}</strong></div></a><button class="text-button add-product" type="button" data-product-id="${product.id}" ${product.priceKes === null ? 'disabled' : ''}>${product.priceKes === null ? 'Details pending' : 'Add to cart'}</button></article>`;
