...
window.renderShell = (activePage = '') => {
  ...
  const navLinks = `<a class="${activePage === 'shop' ? 'active' : ''}" href="./shop.html">Shop phones</a><a class="${activePage === 'about' ? 'active' : ''}" href="./about.html">About us</a><a class="${activePage === 'contact' ? 'active' : ''}" href="./contact.html">Contact</a>`;
  if (header) header.innerHTML = `<div class="header-inner"><a class="logo" href="./index.html" aria-label="Phone Plus home">${logo}</a><nav class="primary-nav" aria-label="Main navigation">${navLinks}</nav><div class="header-actions"><a class="header-icon" href="./contact.html" aria-label="Contact Phone Plus" title="Contact us">${messageIcon}</a><a class="cart-link" href="./cart.html" aria-label="Shopping cart" title="Cart">${cartIcon}<span data-cart-count>0</span></a><details class="mobile-nav"><summary aria-label="Open navigation">${menuIcon}</summary><nav aria-label="Mobile navigation">${navLinks}</nav></details></div></div>`;
  if (footer) footer.innerHTML = `<div><a class="logo" href="./index.html">${logo}</a><p>${config.delivery}</p></div><div class="social-links" aria-label="Social media links"><a href="${config.socialLinks.instagram}" aria-label="Instagram" title="Instagram">${socialIcon.instagram}</a><a href="${config.socialLinks.whatsapp}" aria-label="WhatsApp" title="WhatsApp">${socialIcon.whatsapp}</a><a href="${config.socialLinks.tiktok}" aria-label="TikTok" title="TikTok">${socialIcon.tiktok}</a><a href="${config.socialLinks.facebook}" aria-label="Facebook" title="Facebook">${socialIcon.facebook}</a><a href="${config.socialLinks.x}" aria-label="X" title="X">${socialIcon.x}</a></div><div class="footer-contact"><p>${config.location}</p><a href="./contact.html">Get in touch</a></div>`;
  window.updateCartBadge();
};
window.updateCartBadge = () => document.querySelectorAll('[data-cart-count]').forEach((badge) => { badge.textContent = window.cartCount(); });
window.productVisual = (product) => `<div class="product-visual ${product.condition === 'Ex-UK / Used' ? 'used' : ''}" aria-label="${product.brand} ${product.model}">${product.images && product.images[0] ? `<img src="${product.images[0]}" alt="${product.brand} ${product.model}">` : `<div class="phone-shape"><span>${product.brand}</span></div>`}</div>`;
window.productCard = (product) => `<article class="product-card"><a href="./product.html?id=${product.id}">${window.productVisual(product)}<div class="product-card-body"><span class="condition-tag">${product.condition}</span><h3>${product.brand} ${product.model}</h3><p>${product.storage}${product.ram ? ` · ${product.ram}` : ''}</p><strong>${window.formatPrice(product.priceKes)}</strong></div></a><button class="text-button add-product" type="button" data-product-id="${product.id}" ${product.priceKes === null ? 'disabled' : ''}>${product.priceKes === null ? 'Details pending' : 'Add to cart'}</button></article>`;
window.bindAddButtons = () => document.querySelectorAll('.add-product').forEach((button) => button.addEventListener('click', () => { window.addToCart(button.dataset.productId); window.updateCartBadge(); button.textContent = 'Added to cart'; }));
