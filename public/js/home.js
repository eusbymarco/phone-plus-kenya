document.addEventListener('DOMContentLoaded', () => {
  ...
  if (heroCarousel) {
    ...
    const renderArrivals = () => {
      ...
      heroCarousel.innerHTML = `<div class="arrivals-heading"><div><p class="eyebrow">Just landed</p><h2>New arrivals</h2></div><span>${String(currentPage + 1).padStart(2, '0')} / ${String(Math.max(1, Math.ceil(newProducts.length / pageSize))).padStart(2, '0')}</span></div><div class="arrivals-grid">${pageProducts.map((product) => `<article class="arrival-card"><a href="./product.html?id=${product.id}">${window.productVisual(product)}<div class="arrival-card-info"><span>${product.condition}</span><h3>${product.brand} ${product.model}</h3><strong>${window.formatPrice(product.priceKes)}</strong></div></a><button class="add-product" type="button" data-product-id="${product.id}" ${product.priceKes === null ? 'disabled' : ''}>${product.priceKes === null ? 'Details pending' : 'Add to cart'}</button></article>`).join('')}</div>`;
      window.bindAddButtons();
    };
    renderArrivals();
